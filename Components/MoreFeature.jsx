...
const MoreFeature = () => {
  ...
  return (
    <section className=' glassmorphism flex w-[100%]  flex-col p-4 mt-12 rounded-lg max-w-full '>
            ...
            <h1 className='items-center justify-center flex text-center mt-9 font-extrabold text-4xl  text-gray-800'>Features that make tech Content Creating</h1>
            <h1 className='items-center justify-center flex text-center font-bold text-4xl  bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 bg-clip-text text-transparent'>10x better</h1>
      ...
      { FeatureItems.map((item)=>(
        <div key={item.id} className="p-4 rounded-lg ">
          ...
          <p className="text-base  text-black font-semibold flex justify-start px-11 items-center">{item.description}</p>
        </div>
      )) 
       }
